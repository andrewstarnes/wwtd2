...
namespace DigitalRuby.ThunderAndLightning
{
    ...
    public class LightningBolt
    {
        ...
        public void Initialize(bool useWorldSpace, LightningBoltQualitySetting quality, LightningBoltMeshRenderer lightningBoltRenderer, GameObject parent, LightningBoltScript script,
               FastPool originParticleSystem, FastPool destParticleSystem, ICollection<LightningBoltParameters> parameters)
        {
            // setup properties
        ...
        private void ProcessParameters(LightningBoltParameters p, LightningBoltQualitySetting quality,
               FastPool sourceParticleSystem, FastPool destinationParticleSystem, int maxLights)
        {
        ...
        private void RenderLightningBolt(LightningBoltQualitySetting quality, int generations, Vector3 start, Vector3 end, int groupIndex,
             FastPool originParticleSystem, FastPool destParticleSystem, LightningBoltParameters parameters,
        ...
        private IEnumerator GenerateParticle(FastPool p, Vector3 pos, float delay)
        {
    ...
    public class LightningBoltScript : MonoBehaviour
    {
		...
		public int chains;
		public string targetThisTag;

		public List<GameObject> targetsHit;
		public int maxOnwardChainDist;
		...
		public GameObject LightningOriginParticleSystemPrefab;
		[HideInInspector]
		public FastPool LightningOriginParticleSystem = null;
		public GameObject lightningDestinationParticlePrefab;
		[HideInInspector]
        public FastPool lightningDestinationParticleSystem = null;
        ...
        protected virtual void Start()
        {
			...
			if(LightningOriginParticleSystem.ID==0) {
				LightningOriginParticleSystem = FastPoolManager.GetPool(this.LightningOriginParticleSystemPrefab,true);
			}
			if(this.lightningDestinationParticleSystem.ID==0) {
				this.lightningDestinationParticleSystem = FastPoolManager.GetPool(this.lightningDestinationParticlePrefab,true);
			}
            ...
            for (int i = bolts.Count - 1; i >= 0; i--)
            {
                ...
                if (!bolts[i].Update())
                {
					...
					if(this.chains>0) {
						...
						// Get all targets possible
						GameObject[] g = GameObject.FindGameObjectsWithTag(this.targetThisTag);

						float closest = this.maxOnwardChainDist;
						GameObject closestG = null;

						for(int j = 0;j<g.Length;j++) {
							Vector3 e = this.oneParameterArray[0].End;
							//if(g[
						}
					}
                }
