		...
		if ( reachedLastWpt == false ) { // stop if at last waypoint
			if ( waypoints.Length > 0 ) {
				if ( waypoints[curWpt] ) {
					...
					//turn
					Quaternion _rot = Quaternion.LookRotation( waypoints[curWpt].position - GetComponent<Rigidbody>().position, transform.up );
					GetComponent<Rigidbody>().MoveRotation( Quaternion.RotateTowards( GetComponent<Rigidbody>().rotation, _rot, turnRate * Time.fixedDeltaTime ) );

					//move
					if (GetComponent<Rigidbody>()) {
						GetComponent<Rigidbody>().AddForce( transform.forward * thrust * Time.fixedDeltaTime);
					} else {
