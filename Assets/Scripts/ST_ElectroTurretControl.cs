...
public class ST_ElectroTurretControl : MonoBehaviour {
	...
	public class WeaponData {
		...
		[HideInInspector] public MF_ElectroWeapon script;
	}
			...
			if (_targetRange <= weapons[curWeapon].script.maxRange) {
				...
				if (alternatingFire == true) { // alternate fire bewteen weapons
					...
					if ( Time.time >= lastFire + (weapons[0].script.cycleTime / weapons.Length) ) { 
						...
						weapons[curWeapon].script.DoFire(this.target); // (already checked if weapon is ready)
					...
					for (int sw=0; sw < weapons.Length; sw++) {
						...
						weapons[sw].script.Shoot(this.target); // includes ReadyCheck just in case weapons are different or out of sync
					}
				}
			}
		}
